...
def test_create_question_vote(sqlalchemy_session, app):
    with app.app_context():
        ...
        retrieved_question_vote = QuestionVote.get_question_vote_by_id(
            question_vote.id, session=sqlalchemy_session
        )

        retrieved_question_vote_match = (
            QuestionVote.get_question_vote_by_user_and_question(
                user_id=question_vote.user_id,
                question_id=question_vote.question_id,
        ...
        assert (
            retrieved_question_vote.to_dict() == retrieved_question_vote_match.to_dict()
        )
