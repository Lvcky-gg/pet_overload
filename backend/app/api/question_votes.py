...
def get_user_question_votes():
    ...
    """
    user_question_votes = QuestionVote.query.filter(
    ...
    # list in json format
    question_votes_list = [
        user_question_vote.to_dict() for user_question_vote in user_question_votes
    ]
...
def get_specific_question_vote(question_id):
    ...
    question_vote = QuestionVote.get_question_vote_by_user_and_question(
        int(current_user.id), int(question_id)
    )
